...
import { handleAddItemCart } from '../store/cartProduct' // Import this to clear cart on logout
import { setAddressList } from '../store/addressSlice' // Import this to clear address on logout

const UserMenu = ({close}) => {
   const user = useSelector((state)=> state.user)

   console.log("Current User Role:", user.role); 
   console.log("Is Admin Check:", isAdmin(user.role));
   ...
   const handleLogout = async() => {
        try {
            const response = await Axios({
               ...SummaryApi.logout,
               withCredentials: true // <--- FIX 1: Send cookies to backend
            })
            ...
            // FIX 2: Force logout if token is invalid/expired (401)
