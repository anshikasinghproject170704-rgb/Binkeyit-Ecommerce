...
export async function loginController(request,response){
    try {
        ...
        if(!email || !password){
            return response.status(400).json({
                message : "Provide the email and poassword",
                error : true,
                success : false
            })
        }
...
export async function resetpassword(request,response){
 ...
 export async function refreshToken(request, response) {
    try {
        // 1. FIX: Use request.headers (plural)
        ...
        try {
            ...
            // If jwt.verify fails, it lands here. Return 401, not 500.
            return response.status(401).json({
        ...
        // 3. FIX: Check both 'id' and '_id' to be safe
        // Depending on how you signed the token, it might be in .id or ._id
